...
export const sendEmail = async ({ email, emailType }: ISendMail) => {
    try {
        ...
        const mailOptions = {
            ...
            html: `<p>Click <a href=https://jiffychat.vercel.app/${emailType === "FORGOT_PASSWORD" ? "resetpassword" : "verifyemail"}?${emailType === "USER_VERIFICATION_EMAIL" ? "token" : "forgotToken"}=${randomString}>here</a> to ${emailType === "USER_VERIFICATION_EMAIL" ? "verify your email" : "reset your password"}
            or copy and paste the link below in your browser. <br> ${process.env.DOMAIN_URL}/${emailType === "FORGOT_PASSWORD" ? "resetpassword" : "verifyemail"}?${emailType === "USER_VERIFICATION_EMAIL" ? "token" : "forgotToken"}=${randomString}
            </p>`
        }
